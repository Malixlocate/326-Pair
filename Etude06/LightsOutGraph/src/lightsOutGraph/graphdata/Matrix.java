...
public class Matrix {
	...
	public void calculateInverse(){
		...
		for( int col=0; col<size; col++){
			...
			int row2=row;
			while(row2<size && inverse[row2][col]==0 ) row2++;
			if(row2<size){
...
// vector multiplication
	public int[] getSolution( int[] state ){
		int size = matrix.length;
		if( state.length!=size ) return null;
	...
	public String toString(){
		...
		for( int i=0; i<n; i++ ){
			...
			if( inverse!=null ){
				s += "  ";
				for( int j=0; j<n; j++ ) s += inverse[i][j];
				s += "  ";
				for( int j=0; j<n; j++ ) s += inverse[i][j+n];
			}
			s += "\n";
		}
		if( quiet!=null ){
